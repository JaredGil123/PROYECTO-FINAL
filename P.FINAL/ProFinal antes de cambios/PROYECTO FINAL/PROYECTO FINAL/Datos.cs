    ...
    public partial class Datos : Form
    {
        ...
        private void btnAceptar11_Click(object sender, EventArgs e)
        {
            ...
            if (Resultado == DialogResult.Yes)
            {
                ...
                {
                    fa = (a * Math.Pow(x, 4)) + (b * Math.Pow(x, 3)) + (c * Math.Pow(x, 2)) + (d * x) + f;
                    fb = (a * Math.Pow(y, 4)) + (b * Math.Pow(y, 3)) + (c * Math.Pow(y, 2)) + (d * y) + f;
        ...
        "Error: Intervalo Inválido", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    fxi = (a * Math.Pow(xi, 4)) + (b * Math.Pow(xi, 3)) + (c * Math.Pow(xi, 2)) + (d * xi) + f;
